#!/usr/bin/env python3
"""
Oracle-compatible script to create test user accounts with Free/Premium subscriptions and admin users.
Handles Oracle CLOB fields properly for learning_languages.
"""
...
def create_test_users():
    ...
    test_users = [
        ...
        {"username": "teacher1", "email": "teacher1@desilanguage.com", "full_name": "Language Teacher One", "tier": SubscriptionTier.PRO, "role": UserRole.TEACHER},
        {"username": "superadmin", "email": "superadmin@desilanguage.com", "full_name": "Super Administrator", "tier": SubscriptionTier.PRO, "role": UserRole.ADMIN},
    ]
    ...
    for user_data in test_users:
        try:
            ...
            # Create basic user profile with Oracle-compatible CLOB handling
            # Convert learning_languages list to JSON string for Oracle CLOB
            learning_languages_json = json.dumps(["Telugu", "Hindi"] if user_data["tier"] != SubscriptionTier.FREE else ["Telugu"])
            
            profile = UserProfile(
                user_id=db_user.id,
                bio=f"Test profile for {user_data['full_name']}",
                country="India" if "premium" not in user_data["username"] and user_data["role"] != UserRole.ADMIN else "United States",
                native_language="English",
                learning_languages=learning_languages_json,  # JSON string for Oracle CLOB
    ...
    # Print summary
    print(f"\n{'='*50}")
    print("ORACLE TEST USERS CREATION SUMMARY")
    print(f"{'='*50}")
    ...
    pro_users = [u for u in created_users if u["subscription_tier"] == "pro"]
    admin_users = [u for u in created_users if u["role"] in ["admin", "teacher"]]
    ...
    print(f"\nPro tier users: {len(pro_users)}")
    for user in pro_users:
        print(f"  - {user['username']} ({user['email']})")
    
    print(f"\nAdmin/Teacher users: {len(admin_users)}")
...
if __name__ == "__main__":
    print("Creating Oracle-compatible test user accounts...")
    create_test_users()
    print("\nOracle test user creation completed!")