...
# Enhanced JSON column type optimized for PostgreSQL performance
def get_json_column_type():
    """Return optimized JSON column type for PostgreSQL"""
    # Use JSONB for better performance with indexing and querying
    return JSONB

class DesiLesson(Base):
    __tablename__ = "desi_lessons"
    
    id = Column(Integer, primary_key=True, index=True)  # PostgreSQL auto-increment
...
class DesiVocabulary(Base):
    ...
    target_language_script = Column(String(200), nullable=False)
    transliteration = Column(String(200), nullable=False)
    pronunciation = Column(String(200), nullable=False)
...
class DesiQuizQuestion(Base):
    ...
    options = Column(get_json_column_type(), nullable=False)
...
class UserProfile(Base):
    ...
    learning_languages = Column(get_json_column_type(), default=list)  # List of languages user is learning
    primary_learning_language = Column(String(100), nullable=True)
    learning_goal = Column(String(100), nullable=True)  # e.g., "travel", "business", "family"
...
class UserProgress(Base):
    ...
    level = Column("user_level", Integer, default=1)
...
class LessonCompletion(Base):
    ...
    sections_completed = Column(get_json_column_type(), default=list)  # ["vocabulary", "sentences", "conversations", "quiz"]
...
class Achievement(Base):
    ...
    category = Column(String(50), nullable=False)  # streak, lessons, words, quiz, milestone
    criteria = Column(get_json_column_type(), nullable=False)  # Conditions to unlock
...
class UserAchievement(Base):
    ...
    progress_data = Column(get_json_column_type(), nullable=True)  # Track progress towards achievement
...
class UserSettings(Base):
    ...
    difficulty_preference = Column(String(20), default="adaptive")  # beginner, intermediate, advanced, adaptive
...
class StudySession(Base):
    ...
    activities = Column(get_json_column_type(), default=list)  # Track what user did in session
...
class UserTranslation(Base):
    ...
    from_language = Column(String(10), nullable=False)  # Language code (e.g., "en", "hi")
    to_language = Column(String(10), nullable=False)    # Language code (e.g., "en", "hi")
    from_language_name = Column(String(50), nullable=False)  # Full language name (e.g., "English", "Hindi")
    to_language_name = Column(String(50), nullable=False)    # Full language name (e.g., "English", "Hindi")
    created_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))
...
class DesiStory(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    title = Column(String(500), nullable=True)  # Optional title for the story
    target_language = Column(String(100), nullable=False)
    cefr_level = Column(String(5), nullable=False)  # A1, A2, B1, B2
    scenario = Column(String(500), nullable=True)  # Custom scenario if provided
